            ...
            {visibleTasks.map((task) => (
              <li key={task.id} className="rounded-lg bg-white p-4 shadow">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button disabled={busyTaskId === task.id} onClick={() => updateStatus(task)} className="rounded border px-3 py-1 text-sm hover:bg-gray-50 disabled:opacity-50">{busyTaskId === task.id ? "Updating..." : task.status === "COMPLETED" ? "Mark pending" : "Mark complete"}</button>
                  <button disabled={busyTaskId === task.id} onClick={() => beginEdit(task)} className="rounded border px-3 py-1 text-sm hover:bg-gray-50 disabled:opacity-50">Edit</button>
                  <button disabled={busyTaskId === task.id} onClick={() => deleteTask(task.id)} className="rounded border border-red-200 px-3 py-1 text-sm text-red-600 hover:bg-red-50 disabled:opacity-50">{busyTaskId === task.id ? "Deleting..." : "Delete"}</button>
                </div>
              </li>
            ))}
          </ul>
